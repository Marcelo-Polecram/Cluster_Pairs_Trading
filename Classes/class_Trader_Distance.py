...
class Trader:
    ...
    def threshold_strategy(self, y, x,rf, entry_level=1.0, exit_level=1.0, stabilizing_threshold=5):
        """
        ...
        spread = y - x
        threshold_d = np.std(spread)
        norm_spread = np.asarray(spread.values) 

        # get indices for long and short positions
        longs_entry = norm_spread < -entry_level*threshold_d
        longs_exit = norm_spread > -exit_level*threshold_d
        shorts_entry = norm_spread > entry_level*threshold_d
        shorts_exit = norm_spread < exit_level*threshold_d
        ...
        position_ret, _, ret_summary = self.calculate_position_returns(y, x, num_units)
        ...
        balance_summary = self.calculate_balance(y, x, num_units.shift(1).fillna(0), trading_durations)
        ...
        if np.std(ret_w_costs) == 0:
            ...
            if np.std(position_ret) == 0:
                ...
                sharpe_no_costs = self.calculate_sharpe_ratio(n_years, n_days, position_ret,rf)
            sharpe_w_costs = self.calculate_sharpe_ratio(n_years, n_days, ret_w_costs,rf)

        return summary, (sharpe_no_costs, sharpe_w_costs), balance_summary

    def apply_trading_strategy(self, pairs, train_val_split,rf, strategy='fixed_beta', entry_multiplier=1, exit_multiplier=0,
                               test_mode=False):
        """
        ...
        for i, pair in enumerate(pairs):
            ...
            if strategy == 'fixed_beta':
                summary, sharpe, balance_summary = self.threshold_strategy(y=y, x=x,
                                                                           ...
                                                                           exit_level=exit_multiplier,rf=rf)
                # no costs
    ...
    def add_transaction_costs(self, summary, comission_costs=0.08, market_impact=0.2, short_rental=1):
        ...
        costs = summary.apply(lambda row: self.apply_costs(row, fixed_costs_per_trade, short_costs_per_day),
                              axis=1)
    ...
    def apply_costs(self, row, fixed_costs_per_trade, short_costs_per_day):
        ...
        if row['position_during_day'] == 1. and row['trading_duration'] != 0:
            return fixed_costs_per_trade + fixed_costs_per_trade + short_costs_per_day * \
    ...
    def calculate_balance(self, y, x, positions, trading_durations):
        """
    ...
    def calculate_sharpe_ratio(self, n_years, n_days, ret,rf):
        """
        ...
        """
        # rf = {2014: 0.00033, 2015: 0.00053, 2016: 0.0032, 2017: 0.0093, 2018: 0.0194} RF Americano
        ...
        if year in rf.index:
            sharpe_ratio = (annualized_ret-rf.loc[year][0]) / (np.std(daily_ret)*np.sqrt(time_in_market))
        else:
    ...
    def calculate_portfolio_sharpe_ratio(self, performance, pairs,rf):
        """
        ...
        if year in rf.index:
            ...
            sharpe_ratio = (annualized_ret - rf.loc[year][0]) / (vol*np.sqrt(252))
            ...
            rf_daily = (1+rf.loc[year][0])**(1/252)-1
    ...
    def calculate_maximum_drawdown(self, account_balance):
        ...
        if i == 0:
            ...
            xi = np.arange(0, len(account_balance), len(account_balance)/6)
    ...
    def calculate_position_returns(self, y, x, positions):
        """
        This method receives the series with the positions and calculate the return at the end of each position, not
        yet accounting for costs
    ...
    def return_per_position(self, row):
        if row['end_position'] != 0:
            ...
            return (y_returns - x_returns) * row['positions']
        else:
            return 0
    ...
    def summarize_results(self, sharpe_results, cum_returns, performance, total_pairs, n_years,rf):
        ...
        :param ticker_segment_dict: dict containing segment for each tickera
        ...
        portfolio_sharpe_ratio = self.calculate_portfolio_sharpe_ratio(performance, total_pairs,rf)
        ...
        pairs_df = pd.DataFrame(data, columns=['Leg1', 'Leg2', 't_statistic', 'SSE',
        ...
        results = {'n_pairs': [len(sharpe_results)],
                   'Daily_sharpe_ratio': [portfolio_sharpe_ratio],
                   'avg_annual_roi': [avg_annual_roi],
                   'Total Number of Trades': [pairs_df.positive_trades.sum() + pairs_df.negative_trades.sum()],
                   'Positive Trades': [pairs_df.positive_trades.sum()],
                   'Negative Trades': [pairs_df.negative_trades.sum()],
                   'pct_positive_trades_per_pair': [avg_positive_trades_per_pair_pct],
                   'pct_pairs_with_positive_returns': [positive_pct],
