...
class SeriesAnalyser:
    ...
    def check_properties(self, train_series, test_series, min_half_life=78, max_half_life=20000,
        ...
        pairs = [(X, Y), (Y, X)]
        pair_stats = [0] * 2
        criteria_not_verified = 'distance'
        '''
        ...
        for i, pair in enumerate(pairs):
            ...
            # Calc SSEs
            sse = sum(np.power(pair[0] - pair[1], 2))
            
            spread = pair[1] - pair[0] # as Pandas Series
    ...
    def find_pairs(self, data_train, data_test, min_half_life=78, max_half_life=20000,
                   min_zero_crossings=0, hurst_threshold=0.5, subsample=0,top_n=20):
        """
        ...
        # Sort cointegrated pairs by a specific criterion from the result (e.g., spread half-life)
        sorted_pairs = sorted(pairs, key=lambda x: x[2]['SSE'])


        return sorted_pairs[:top_n], pairs_fail_criteria
    ...
    def apply_PCA(self, n_components, df, svd_solver='auto', random_state=0):
        ...
        # standardize
        X = np.asarray(pca.components_.T, dtype=np.float128)
        X = preprocessing.StandardScaler().fit_transform(X)
        
        # X = preprocessing.StandardScaler().fit_transform(pca.components_.T) Formula antiga
    ...
    def get_candidate_pairs(self, clustered_series, pricing_df_train, pricing_df_test,top_n, min_half_life=78,
                            max_half_life=20000, min_zero_crosings=20, hurst_threshold=0.5,
                            subsample=0):
        """
